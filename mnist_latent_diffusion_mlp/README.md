# MNIST Latent Diffusion (MLP Autoencoder)

Latent Diffusion Model for MNIST digit generation using the **MLP-based autoencoder**. Unlike the pixel-space diffusion in `mnist_single_digit`, this implementation operates in the latent space of a pretrained MLP autoencoder.

**Note**: This is the MLP version. For the convolutional autoencoder version, see `mnist_latent_diffusion_conv/`.
...
### 1. Train Autoencoder (if not done yet)

```bash
cd ../mnist_ae
python train.py --epochs 25 --generate-images
```
